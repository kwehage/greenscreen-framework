        ...
        for element in ghs_data_files:
            ...
            for i in range(1, book.nsheets):
                ...
                for element in calculated_fields:
                    exec('%s.%s.%s="%s"' %
                         ('ghs_japan_data', ghs_data_ind, element[0], element[1]))
                        ...
                        for j in range(len(hzd_traits[i][0])):
                            ...
                            for sub in hzd_sub_traits:
                                ...
                                if sub[0] == "hazard_name":
                                    current_cas['hazards'][hzd_traits[i][0][j]][
...
def jsonify_hazard(json_data, cas_data, name, CAS, hazard, category, criteria):
    ...
    ## Program execution should arrive here, which indicates that the program has
    #  not encountered duplicate sources when translating from the GHS Japan
    #  database
    else:
        systemic_words = (['respiratory', 'blood', 'kidney', 'liver', 'adrenal',
                           'gastro', 'systemic', 'eye', 'heart', 'bone',
                           'hematop', 'cardio', 'spleen', 'thyroid', 'lung',
                           'gingi', 'testes'])
...
def save_xml_individual(xml_data, data_dir, list_type):
    ...
    for element in data_dir_split:
        if not element == '':
            ...
            os.chdir(element)
    os.chdir(cwd)
    etree.strip_attributes(xml_data,
